...
class KeywordTriggerFormWrapper(KeywordTriggerForm):
    def __init__(self, user,  *args, **kwargs):
        super(KeywordTriggerFormWrapper, self).__init__( remove_flow = True,user = user,  *args, **kwargs)
...
class TriggerCRUDL(SmartCRUDL):
    ...
    class Register(CreateTrigger):
        ...
        def form_valid(self, form):
            ...
            t_to_flow = TriggerToFlow.objects.create(trigger = trigger, flow = group_flow)
            channels = channels.objects.filter(org = org)
            t_to_flow.channels.add(*channels)
    ...
    class Schedule(CreateTrigger):
        ...
        def form_valid(self, form):
            ...
            t_to_flow = TriggerToFlow.objects.create(trigger = trigger, flow =self.form.cleaned_data['flow'])
            channels = channels.objects.filter(org = self.request.user.get_org())
            t_to_flow.channels.add(*channels)
            for group in recipients['groups']:
                trigger.groups.add(group)
    ...
    class MissedCall(CreateTrigger):
        ...
        def form_valid(self, form):
            ...
            t_to_flow = TriggerToFlow.objects.create(trigger = trigger, flow = form.cleaned_data['flow'])
            channels = channels.objects.filter(org = org)
            t_to_flow.channels.add(*channels)
