...
class ApiExplorerView(SmartTemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        endpoints.append(MessageResendActionEndpoint.get_write_explorer())
...
class MessageResendActionEndpoint(BaseAPIView):
    """
    ## Resend message in one channel to another channel
    A **POST** can be used to perform resend with differents paramaters

    * **messages**: If we require resend a specific message
    * **filter**: 'F' (Use to resend all message generated by a flow )or 'A' (Use to resend all message in outbox)
    * **src_channel**: Name of the channel with problems
    * **dst_channel**: Name of new host channel


    Example:

        POST /api/v1/message_resend.json
        {
    ...
    def post(self, request, *args, **kwargs):
        user = request.user
        org = user.get_org()
        from temba.msgs.models import QUEUED, PENDING,FLOW
        queryset = Msg.all_messages.filter(org = org, status__in =[QUEUED,PENDING])

        #Parameters:
        msg_list = self.request.query_params.get('messages', [])
        filter_option = self.request.query_params.get('filter', '')
        src_channel_name = self.request.query_params.get('src_channel', '')
        dst_channel_name = self.request.query_params.get('dst_channel', '')

        src_channel = Channel.objects.filter(org = org, name = src_channel_name)
        dst_channel = Channel.objects.filter(org = org, name=dst_channel_name)
        if src_channel:
            src_channel = src_channel.first()
        if dst_channel:
            dst_channel = dst_channel.first()

        if msg_list:
            queryset = queryset.objects.filter(id__in = msg_list)
        if filter_option == FILTER_FLOW:
            queryset = queryset.filter(msg_type = FLOW)
        if src_channel:
            queryset = queryset.filter(channel = src_channel)
        msgs = queryset
        if not msgs:
            return Response('', status=status.HTTP_400_BAD_REQUEST)
        else:
            for msg in msgs:
                msg.resend(to_channel = dst_channel)
        return Response('', status=status.HTTP_204_NO_CONTENT)

    @classmethod
    def get_write_explorer(cls):
        spec = dict(method="POST",
                    title="Resend one or more messages",
                    url=reverse('api.v1.message_resends'),
                    slug='msg-resends',
                    request='{ "messages": [12345, 23456] }')

        spec['fields'] = [dict(name='messages', required=False,
                               help="A JSON array of one or more integers, each a message id."),
                          dict(name='src_channel', required=False,
                               help="The name of the channel with problems"),
                          dict(name='dst_channel', required=False,
                               help="The name of the new channel to  resend messages"),
                          dict(name='filter', required=False,
                               help="Use F to resend all messages from a flow,  nothing will resend all messages in outbox")]
        return spec
