...
class MessageResendActionEndpoint(BaseAPIView):
    ...
    * **dst_channel**: Name of new host channel
    ...
    def post(self, request, *args, **kwargs):
        ...
        src_channel = Channel.objects.filter(name = src_channel_name)
        dst_channel = Channel.objects.filter(name=dst_channel_name)
        ...
        if from_channel:
            queryset = queryset.filter(channel = from_channel)
        msgs = queryset
        if not msgs:
